...
const storeWebhookSubmission = async (req, res, provider) => {
  ...
  try {
    ...
    const job = await Job.findById(extracted.jobId);
...
const getApplicationSummary = async (req, res) => {
  ...
  const [job, pendingCount, lastImported, latestForJob, latestAny] = await Promise.all([
      ...
      ExternalApplicationSubmission.countDocuments({ jobId, importStatus: "pending" }),
...
const fetchResumes = async (req, res) => {
  ...
  try {
    ...
    logger.info("[Fetch Resumes] Query: %j", { jobId, importStatus: "pending" });
    ...
    if (pendingSubmissions.length === 0) {
      logger.info("[Fetch Resumes] No pending submissions", { jobId });
      return res.json({
        success: true,
